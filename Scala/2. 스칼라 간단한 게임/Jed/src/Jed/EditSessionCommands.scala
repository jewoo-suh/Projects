...
object EditSessionCommands extends Logging.Loggable {
  ...
  //Definitions for Copy, Paste, and Flip 

  def copyHL() : SessionCommand = new SessionCommand {
    def DO(session: EditSession): StateChangeOption = {
      session.copy();
      None
    }
  ...
  def pasteHL() : SessionCommand = new SessionCommand {
    def DO(session: EditSession): StateChangeOption = {
      session.paste();
      None
    }
  ...
  def flip() : SessionCommand = new SessionCommand {
    def DO(session: EditSession): StateChangeOption = {
      session.flip();
      None
    }
